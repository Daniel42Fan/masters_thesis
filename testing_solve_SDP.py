...
def testing_Minimize_trace_rho_times_h_cg(ham, W, N=4, dim=2, chi=3, output=True):
    ...
    prob, rho, constraints = ssdp.Minimize_trace_rho_times_h_cg(
        ham, W, N=N, dim=dim, chi=chi, output=output
    )
...
for s in range(1, 2):
    ...
    prob, rho, constraints = testing_Minimize_trace_rho_times_h_cg(
        ham, W, N=N, dim=dim, chi=chi, output=output
    )
...
def testing_get_vectors_and_matrices_cg(N, ham, W, dim=2, chi=3):
    c_vec, x_vec, b_vec, A_mat, B_mat, basis_vec = ssdp_cg.get_vectors_and_matrices_cg(
        N, ham, W, dim=dim, chi=chi
    )
...
def testing_Minimize_cvec_timec_xvec_cg(N, ham, W, dim=2, chi=3):
    prob, optimal_variable, optimal_dual_variable, constraints = (
