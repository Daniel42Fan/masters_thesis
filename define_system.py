...
def Build_H_TFIM_Interaction_Term(h_z=1):
    ...
    interaction_term = -sparse.kron(sx, sx) - 1 / 2 * (h_z / 2) * (
        sparse.kron(sz, Id) + sparse.kron(Id, sz)
    )
...
def Build_H_XXZ_full(M, dim=2, Delta=1):
    ...
    for i in range(0, k):
        operator = np.kron(
            np.kron(np.identity(dim**i), ham), np.identity(dim ** (k - i - 1))
        )
        op.append(operator)
...
def Build_Single_Spin_Ops(N):
    ...
    for i in range(0, N):
        sxi.append(
            sparse.kron(
                sparse.kron(sparse.identity(2 ** (i)), sx),
                sparse.identity(2 ** (N - i - 1)),
            )
        )
        syi.append(
            sparse.kron(
                sparse.kron(sparse.identity(2 ** (i)), sy),
                sparse.identity(2 ** (N - i - 1)),
            )
        )
        szi.append(
            sparse.kron(
                sparse.kron(sparse.identity(2 ** (i)), sz),
                sparse.identity(2 ** (N - i - 1)),
            )
        )
