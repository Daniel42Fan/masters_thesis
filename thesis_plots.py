...
def plot_energies(model, map, *args, normalize=True, xscale='log', yscale='log'):
    ...
    for Delta in range(1, 2):
        for N in range(5, 7, 2):
            ...
            '''
            if normalize==True:
                ax.set_title('Normalized Energies of a Random Hamiltonian\nN = {}, Delta = {}'.format(N, Delta),
        ...
        for N in range(5, 7, 2):
            ...
            for i in range(len(e_after_gd_list)):
                ...
                if xscale=='log':
                    ...
                    pos_e_after = []
                    pos_e_before = []
                    for e_before, e_after in zip(e_before_gd_list[i], e_after_gd_list[i]):
                        if e_after >= 0:
                            pos_e_before.append(e_before)
                            pos_e_after.append(e_after)
                    ax.plot(pos_e_before, pos_e_after, marker=markers[i],
            ...
            ax.text(x_left-0.018*(x_dis), e_N+0.02*(y_dis),
                    r'$E_{{ \mathrm{{ {} }} }}({})$'.format('LTI', N),
                    color='k')
            ax.hlines(y=e_N_minus_1, xmin=-1, xmax=0, colors='darkred',
                        linestyle='--', linewidth=1)#, label='E_{}'.format(N))
            ax.text(x_left+0.05*(x_dis), e_N_minus_1+0.02*(y_dis),
                    r'$E_{{ \mathrm{{ {} }} }}({})$'.format('LTI', N-1),
                    color='darkred')
            ax.vlines(e_N_minus_1, ymin=-1, ymax=1 , colors='darkgreen',
                        linestyles='--', linewidth=1)#, label='E_{}'.format(N-1))
            ax.text(x_left-0.018*(x_dis), y_lower-0.01*(y_dis),#ax.text(e_N_minus_1+0.01*(x_dis), y_lower-0.01*(y_dis),
                    r'$E_{{ \mathrm{{ {} }} }}({})$'.format('LTI', N-1),
                    color='darkgreen')
            ax.vlines(e_N, ymin=-1, ymax=1 , colors='purple',
                        linestyles='--', linewidth=1)#, label='E_{}'.format(N))
            ax.text(e_N-0.07*(x_dis), y_lower-0.01*(y_dis),
                    r'$E_{{ \mathrm{{ {} }} }}({})$'.format('LTI', N),
                    color='purple')
            ...
            if normalize==True:
                ...
                ax.set_xlabel('Accuracy Before Optimizing $E_{\text{LTI}}(n,W)$ over $W$')
                ax.set_ylabel('Accuracy After Optimizing $E_{\text{LTI}}(n,W)$ over $W$')
                plt.savefig(path+'normalized_energies_Delta{}_N{}.pdf'.format(Delta, N))
            else:
                ax.set_title('Initial and Final Energy Value for XX Model\nn = {}'.format(N),
                                fontsize=BIGGER_SIZE)
                ...
                if chi_start==1:
                    if xscale=='log':
                        plt.savefig(path+'log_energies_all_chis_Delta{}_N{}.pdf'.format(Delta, N))
                    else:
                        plt.savefig(path+'energies_all_chis_Delta{}_N{}.pdf'.format(Delta, N))
                else:
                    if xscale=='log':
                        plt.savefig(path+'log_energies_Delta{}_N{}.pdf'.format(Delta, N))
                    else:
                        plt.savefig(path+'energies_Delta{}_N{}.pdf'.format(Delta, N))
                    pass
            print('Plot saved successfully')
...
figs, axes = plot_energies2('XXZ/', 'random_maps/', normalize=False, xscale='log', yscale='log')
    ...
    for Delta in range(0, 1):
        ...
        for N in range(5, 7, 2):
            ...
            for chi in range(2, 3):
                ...
                print('Plotting intermediate energy values of gradient search for Delta = {}, N = {}, chi = {}'.format(Delta, N, chi))
                fig, ax = create_plot_template()
                #ax.set_title('Intermediate Results of the Gradient Descent Using the XXZ Model\nn = {}, $\Delta = {}$, $\chi = {}$'.format(N, Delta, chi),
                #                fontsize=BIGGER_SIZE)
                ax.set_title('Intermediate Energy Values of the Gradient Search for the XX Model\nn = {}, $\chi = {}$'.format(N, chi),
                                fontsize=BIGGER_SIZE)
                # plot first 10 maps
                for i in range(10):
                ...
                x_dis = x_right - x_left
                y_dis = y_upper - y_lower
                ax.hlines(y=e_N, xmin=-10, xmax=1000, colors='k',
                            linestyle='--', linewidth=1)#, label='E_{}'.format(N))
                ax.text(x_left+0.05*(x_dis), e_N+0.01*(y_dis),
                        r'$E_{{ \mathrm{{ {} }} }}({})$'.format('LTI', N),
                        fontsize='x-large', color='k')
                ax.hlines(y=e_N_minus_1, xmin=-10, xmax=1000, colors='darkred',
                            linestyle='--', linewidth=1)#, label='E_{}'.format(N))
                ax.text(x_left+0.05*(x_dis), e_N_minus_1+0.01*(y_dis),
                        r'$E_{{ \mathrm{{ {} }} }}({})$'.format('LTI', N-1),
                        fontsize='x-large', color='darkred')
        ...
        for N in range(5, 7, 2):
            ...
            if model=='random_Hamiltonian/':
                ...
                for s in args[0]:
                    ...
                    for chi in range(2, 8):
                        # get data
                        e_before_gd, e_after_gd = ad.get_energies(Delta, N, chi, s, model=model, map=map, normalize=normalize)
                        if normalize==True:
                            '''
            ...
            for i, s in zip(range(len(positions)), args[0]):
                ax.scatter([], [], c=standard_colors[i], label='Random Hamiltonian seed {}'.format(s))
            plt.legend()
            ax.set_xticks(p)
            ...
            #            fontsize=BIGGER_SIZE)
            ax.set_title('Accuracy of the Ground State Energy of Random Hamiltonians\nn = {}'.format(N),
                        fontsize=BIGGER_SIZE)
    ...
    for Delta in range(1, 2):
        for N in range(5, 7, 2):
            ...
            for chi in range(2, 3):
                ...
                ax.set_title('Spectrum of the {}-body Heisenberg Model Hamiltonian and the Coarsed-Grained Hamiltonian\n$ \chi = {}$'.format(N-2, chi),
                            fontsize=MEDIUM_SIZE)
                labels = ['Ground State of the Optimized Coarsed-Grained Hamiltonian',
                ...
                labels0 = ['Ground State of the Coarsed-Grained Hamiltonian',
                ...
                ax.hlines(y=eigvals_H, xmin=0, xmax=len(eigvals), colors='k', linestyle='--', label='Spectrum of the XX Hamiltonian')
                ...
                plt.savefig(path+'subspace_energies_Delta{}_N{}_chi{}.pdf'.format(Delta, N, chi))
                print('saved')
                plt.close("all")
                #plt.show()
                    #except:
