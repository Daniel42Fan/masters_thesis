...
'''
def normalized_energies_to_absolute_values(Delta, N, chi, ham, *args,
                                dim=2, model='XXZ/', map='', normalize=True):
    prob_N_minus_1, prob_N = ssdp.compare_solutions_no_cg(N-1, N, ham,
                                                    dim=dim, output=False)
    ...
    e_before_gd_n, e_after_gd_n  = get_energies(Delta, N, chi, *args,
                                        model=model, map=map, normalize=normalize)
...
'''
def save_converted_energies(Delta, N, chi, ham, *args,
                dim=2, model='XXZ/', map='', normalize=True):
    # convert data
    e_before_gd, e_after_gd = normalized_energies_to_absolute_values(Delta, N, chi, ham, *args,
                                dim=dim, model=model, map=map, normalize=normalize)
    print('e_before_gd =\n', e_before_gd)
    print('e_after_gd =\n', e_after_gd)
...
def get_res_history(Delta, N, chi, *args, model='XXZ/', map='', normalize=False):
    path = get_data_path2(Delta, N, chi, *args,
                model=model, map=map, normalize=normalize)
...
def get_optimized_cg_map(Delta, N, chi, *args, dim=2, model='XXZ/', map='', normalize=False):
    print(args)
    print(*args)
    path = get_data_path2(Delta, N, chi, *args,
                model=model, map=map, normalize=normalize)
...
'''
def investigate_optimized_maps(Delta, N, chi,  dim=2, *args, model='XXZ/', map='', normalize=False):
    # get the optimized maps
    W = get_optimized_cg_map(Delta, N, chi, dim=dim, *args,
                            model=model, map=map, normalize=normalize)
    # get the energies after gradient descent
    e_before_gd, e_after_gd = get_energies(Delta, N, chi,
                                    model=model, map=map, normalize=normalize)
...
def calc_q_dagger_H_q(Delta, N, chi, ham, model, map, *args, dim=2):
    ...
    W0 = [dsdp_cg.generate_linear_map_N(N=N, dim=dim, chi=chi, seed=seed) for seed in range(100) if seed not in error_seeds]
    #print('len(W0)', len(W0))
    H = ham
