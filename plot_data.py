    ...
    for Delta in range(1, 2):
        for N in range(5, 7, 2):
            ...
            prob_N_minus_1, prob_N = ssdp.compare_solutions_no_cg(
                N - 1, N, args[0], dim=2, output=True
            )
        ...
        for N in range(7, 9, 2):
            for chi in range(1, N):
                try:
                    ...
                    print(
                        "Plotting heatmaps for Delta = {}, N = {}, chi = {}".format(
                            Delta, N, chi
                        )
                    )
                    ...
                    plt.colorbar()
                    plt.title(
                        "Subspace angles between different coarse graining maps for\n\Delta = {}, N = {}, \chi = {}".format(
                            Delta, N, chi
                        )
                    )
                    # plt.xlabel('')
                    # plt.ylabel('')
                    plt.savefig(
                        "plots/subspace_angles/"
                        + model
                        + "subspace_angles_Delta{}_N{}_chi{}.pdf".format(Delta, N, chi)
                    )
                    plt.close("all")
                except:
                    print("Map not found")
    ...
    for Delta in range(0, 1):
        for N in range(5, 7, 2):
            ...
            for chi in range(2, 3):
                ...
                ax.set_title(
                    "$N = {}, \Delta = {}, \chi = {}$".format(N - 2, Delta, chi)
                )
    ...
    for Delta in range(0, 1):
        ...
        for N in range(5, 7, 2):
            for chi in range(2, 3):
                ...
                print(
                    "Plotting intermediate energy values of gradient descent for Delta = {}, N = {}, chi = {}".format(
                        Delta, N, chi
                    )
                )
                fig, ax = create_plot_template()
                ax.set_title(
                    "Intermediate Energy Values of the Gradient Search for the XXZ Model\nN = {}, $\Delta = {}$, $\chi = {}$".format(
                        N, Delta, chi
                    )
                )
                # plot first 10 maps
            ...
            for chi in range(1, N):
                try:
                    for g in g_tol:
                        g = "g_tol" + str(g) + "/"
                        e_before_gd, e_after_gd = ad.get_energies(
                            Delta, N, chi, g, model=model, map=map
                        )
                        e_after_gd = 1 - np.asarray(e_after_gd)
                        e_before_gd = 1 - np.asarray(e_before_gd)
                        e_before_gd_list.append(e_before_gd)
                    ...
                    for i in range(len(e_before_gd_list)):
                        ax.plot(
                            ...
                            marker="x",
                            linestyle="None",
                            label="g_tol = {}".format(g_tol[i]),
                        )
                    ax.set_title(
                        "Comparing gtol parameter for\nN = {}, Delta = {}, chi = {}".format(
                            N, Delta, chi
                        )
                    )
                    ...
                    plt.savefig(
                        path + "energies_Delta{}_N{}_chi{}.pdf".format(Delta, N, chi)
                    )
                    print("Plot saved successfully")
                    plt.close("all")
                except:
