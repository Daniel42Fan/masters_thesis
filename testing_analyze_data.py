...
def testing_normalized_energies_to_absolute_values(Delta, N, chi, ham, *args,
                                dim=2, model='XXZ/', map='', normalize=True):
    e_before_gd, e_after_gd = ad.normalized_energies_to_absolute_values(Delta, N, chi, ham, *args,
...
def testing_save_converted_energies(Delta, N, chi, ham, *args,
                dim=2, model='XXZ/', map='', normalize=True):
    ad.save_converted_energies(Delta, N, chi, ham, *args,
                    dim=dim, model=model, map=map, normalize=normalize)
    return 0
...
def testing_get_optimized_cg_map(Delta, N, chi, dim=2,
                                *args, model='XXZ/', map='', normalize=False):
    W = ad.get_optimized_cg_map(Delta, N, chi, dim=dim, *args,
...
def testing_investigate_optimized_maps(Delta, N, chi, dim=2, *args,
                                        model='XXZ/', map='', normalize=False):
    x = ad.investigate_optimized_maps(Delta, N, chi, dim=dim, *args,
                                        model=model, map=map, normalize=normalize)
    return 0
'''
...
def testing_get_error_text(Delta, N, chi, *args, model='XXZ/', map='', normalize=False):
    errors = ad.get_error_txt(Delta, N, chi, *args, model=model, map=map, normalize=normalize)
