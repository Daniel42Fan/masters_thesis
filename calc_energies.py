...
def profiling_cp_vs_np_ptrace(dim=2):
    ...
    ptr_rho3_cp2 = cp.partial_trace(
        ptr_rho3_cp1, list(np.ones(8, dtype="int") * dim), axis=0
    )
    # tracing out third subsystem
    ptr_rho3_cp3 = cp.partial_trace(
        ptr_rho3_cp2, list(np.ones(7, dtype="int") * dim), axis=0
    )
    # tracing out sixth subsystem
    ptr_rho3_cp6 = cp.partial_trace(
        ptr_rho3_cp3, list(np.ones(6, dtype="int") * dim), axis=2
    )
    # tracing out ninth subsystem
    ptr_rho3_cp9 = cp.partial_trace(
        ptr_rho3_cp6, list(np.ones(5, dtype="int") * dim), axis=4
    )
    t1 = time.time()
    total_time = t1 - t0
    ...
    print("timing using np ptr")
    t0 = time.time()
    ptr_rho3_NE_np = qt_gf.partial_trace(
        rho3, [1, 2, 3, 6, 9], list(np.ones(9, dtype="int") * dim)
    )
    t1 = time.time()
    total_time = t1 - t0
    print("total time using np ptr =", timedelta(seconds=total_time))
    ptr_mat_np = qt_gf.partial_trace(
        mat, [1, 2, 3, 6, 9], list(np.ones(9, dtype="int") * dim)
    )
    """
    ...
    *args,
    ...
    for i in range(seed_start, seed_end):
        ...
        if linear_map == "pertubation":
            ...
            W, path = linear_map_random(
                N, dim, chi, Delta, seed, model, *args, normalize=normalize
            )
        else:
            print("Chosen linear map is not valid. Please chose a valid linear map.")
            print("Code execution will be stopped.")
            sys.exit()
        prob, rho, __ = ssdp.Minimize_trace_rho_times_h_cg(
            ham, W, N=N, dim=dim, chi=chi, output=False
        )
        # norm = ssdp.calc_norm_cg_rho(rho, W, dim=dim)
        # print('\nnorm', norm)
        try:
            res, history, g_tol = ssdp_cg.gradient_descent(
                N, W, ham, g_tol=g_tol, dim=dim, chi=chi
            )
            """
            ...
            if normalize == True:
                e, shift, norm_factor = ssdp.compare_normalized_solutions(
                    N - 1, N, prob.value, -res.fun, ham, dim=dim
                )
...
def execute_calc_energies(dim=2):
    ...
    for seed_ham in ham_seeds:
        ...
        for d in range(2, 3):
                ...
                for chi in range(2, 5):
                    ...
                    for g in g_tol:
                        ...
                        (
                            ...
                            linear_map,
                            Delta,
                            {"n_states": 4, "seed_Hamiltonian": seed_ham},
                            dim=2,
                            seed_end=100,
                            g_tol=g,
                            model=model,
            ...
            total_time = t1 - t0
            print(
                "total time for Delta = {}, N = {}:".format(Delta, N),
...
def linear_map_pertubation(H, N, dim, chi, Delta, seed):
    ...
    path = "test/data/XXZ/pertubation/Delta{}_N{}_chi{}_epsilon{}/".format(
        Delta, N, chi, epsilon
    )
    return W, path
...
def linear_map_n_lowest_states(H, N, chi, Delta, n_states, seed):
    ...
    path = "test/data/XXZ/{}_lowest_states/Delta{}_N{}_chi{}/".format(
        n_states, Delta, N, chi
    )
    return W, path
