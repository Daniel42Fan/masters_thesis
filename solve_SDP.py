...
def compare_solutions_no_cg(N_minus_1, N, ham, dim=2, output=False):
    ...
    prob_N_minus_1 = Minimize_trace_rho_times_h(
        ham, rhos_N_minus_1, N=N_minus_1, output=output
    )
...
def compare_normalized_solutions(
    N_minus_1, N, energy_no_gd, energy_gd, ham, dim=2, output=True
):
...
def normalize_history(N_minus_1, N, prob_value, history, ham, dim=2, output=False):
    ...
    """
    e, shift, norm_factor = compare_normalized_solutions(
        N_minus_1, N, prob_value, -history[0], ham, dim=2, output=False
    )
...
def Minimize_trace_rho_times_h_cg_2D(ham, W, dim=2, chi=1):
    ...
    """
    ptr_rho3_NE = ce.partial_trace_cp(
        rho3, [1, 2, 3, 6, 9], list(np.ones(9, dtype="int") * dim)
    )
    ptr_rho3_NW = ce.partial_trace_cp(
        rho3, [3, 2, 1, 4, 7], list(np.ones(9, dtype="int") * dim)
    )
    ptr_rho3_SE = ce.partial_trace_cp(
        rho3, [3, 6, 9, 8, 7], list(np.ones(9, dtype="int") * dim)
    )
    ptr_rho3_SW = ce.partial_trace_cp(
        rho3, [1, 4, 7, 8, 9], list(np.ones(9, dtype="int") * dim)
    )
    ...
    """
    ptr_omega4_NE = ce.partial_trace_cp(
        omega4, [1, 2, 3, 4, 6, 8, 12], list(np.ones(12, dtype="int") * dim) + [chi]
    )
    ptr_omega4_NW = ce.partial_trace_cp(
        omega4, [1, 2, 3, 4, 5, 7, 9], list(np.ones(12, dtype="int") * dim) + [chi]
    )
    ptr_omega4_SE = ce.partial_trace_cp(
        omega4, [4, 6, 8, 9, 10, 11, 12], list(np.ones(12, dtype="int") * dim) + [chi]
    )
    ptr_omega4_SW = ce.partial_trace_cp(
        omega4, [1, 5, 7, 9, 10, 11, 12], list(np.ones(12, dtype="int") * dim) + [chi]
    )
    ...
    perm_rho3_NE = qt_gf.syspermute(
        rho3, [1, 4, 7, 8, 9, 2, 3, 5, 6], list(np.ones(9, dtype="int") * dim)
    )
    perm_rho3_NW = qt_gf.syspermute(
        rho3, [3, 6, 7, 8, 9, 1, 2, 4, 5], list(np.ones(9, dtype="int") * dim)
    )
    perm_rho3_SE = qt_gf.syspermute(
        rho3, [1, 2, 3, 4, 7, 5, 6, 8, 9], list(np.ones(9, dtype="int") * dim)
    )
    perm_rho3_SW = qt_gf.syspermute(
        rho3, [1, 2, 3, 6, 9, 4, 5, 7, 8], list(np.ones(9, dtype="int") * dim)
    )
