...
V_mat_L = testing_Build_matrix_rep(
    lin_op, basis_vec_in, basis_vec_out, "L", dim, chi, W
)
V_mat_R = testing_Build_matrix_rep(
    lin_op, basis_vec_in, basis_vec_out, "R", dim, chi, W
)
...
def check_dB():
    ...
    for index_tuple in indices:
        ...
        E_L, E_L_dagger, E_R, E_R_dagger = dsdp_cg.get_E_matrices(
        ...
        for i in range(0, n_cols):
            for j in range(0, n_rows):
                # use product rule
                mat_L[i][j] = np.trace(
                    (E_L @ basis_vec_in[j] @ V_L_dagger).conj().T @ basis_vec_out[i]
                ) + np.trace(
                    (V_L @ basis_vec_in[j] @ E_L_dagger).conj().T @ basis_vec_out[i]
                )

                mat_R[i][j] = np.trace(
                    (E_R @ basis_vec_in[j] @ V_R_dagger).conj().T @ basis_vec_out[i]
                ) + np.trace(
                    (V_R @ basis_vec_in[j] @ E_R_dagger).conj().T @ basis_vec_out[i]
                )
    ...
    """
    B_mat = dsdp_cg.Build_B_matrix_cg(
        N, W, dim=dim, chi=chi, gradient=False, grad_kl=False, index_tuple=(0, 0)
    )
    print("B_mat.shape", B_mat.shape)
    grad_B = dsdp_cg.Build_B_matrix_cg(
        N, W, dim=dim, chi=chi, gradient=True, grad_kl=False, index_tuple=(0, 0)
    )
    print("grad_B.shape", grad_B.shape)
    return grad_B
