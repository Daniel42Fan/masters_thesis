...
def linear_op_cg_maps(
    basis, *args
):  # , grad_descent=False, index_tuple=(0,0), roll=0):
    """
...
def Build_matrix_rep(lin_op, basis_in, basis_out, *args, **kwargs):
    ...
    for i in range(0, n_cols):
        for j in range(0, n_rows):
            ...
            mat[i][j] = np.trace(
                lin_op(basis_in[j], args, kwargs).conj().T @ basis_out[i]
            )
    return mat
...
def get_derivative_V(N, W, dim=2, chi=3):
    ...
    for ind in indices:
        # print('indices' ,ind)
        # roll = 0
        V_L, V_R = dsdp_cg.get_V_matrices(
            N, W, dim=dim, chi=chi, grad_descent=grad_descent, index_tuple=ind, roll=0
        )
        ...
        # roll = 1
        V_L2, V_R2 = dsdp_cg.get_V_matrices(
            N, W, dim=dim, chi=chi, grad_descent=grad_descent, index_tuple=ind, roll=1
        )
